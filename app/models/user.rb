class User < ApplicationRecord
  ...
  enum job: { holiday: 0, tc_sales: 1, life_coach: 2, mentor: 3, carrier: 4, invisible: 5, all_day: 6 }

  def self.working(wday)

    # holiday, salesで無い→休日がwday出ない→もしくはentry?がtrue の人が基本出席
    base = (where(entry?: nil).where.not(job: [ :holiday, :tc_sales, :invisible]) - joins(:holidays)
  ...
  def self.holiday(wday)
    ...
    where.not(job: [:invisible]) - working(wday) | where(entry?: false)
  end
